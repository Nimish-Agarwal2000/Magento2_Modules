...
class Manage extends \Magento\Framework\View\Element\Template {
     ...
     * @param array $data            
     */
    public function __construct(Template\Context $context, Collection $productFactory, \Magento\Directory\Model\Currency $currency, StockRegistry $stockRegistry, \Dotsquares\Marketplace\Helper\System $systemHelper, \Magento\Framework\Message\ManagerInterface $messageManager, array $data = []) {
    ...
    /**
     * Get Manage product price symbol
     *
     * @return string
     */
    public function getCurrentCurrencySymbol()
{
     ...
     * @return string url
     */
    public function getPrductAddUrl($productId) {
        return $this->getUrl ( 'dailydeal/product/add' ) . 'product_id/' . $productId;
    }
    public function getPrductEditUrl($productId) {
        return $this->getUrl ( 'dailydeal/product/edit' ) . 'product_id/' . $productId;
    }
    ...
    public function getFilterProducts() {
        ...
        if (isset( $productIds ) && $delete == 'delete') {
            $deleteFlag = 0;
